...
Page({
      ...
      if (res.errno === 0) {
        ...
        if (_specificationList.length == 1) {
          if (_specificationList[0].valueList.length == 1) {
            ...
            // 这里检测一下
            let _productPrice = res.data.stockList[0].price;
            let _goodsPrice = res.data.info.currentPrice;
  ...
  changeSpecInfo: function() {
    ...
    if (this.isCheckedAllSpec()) {
      ...
      if (checkedProduct.count > 0) {
        this.setData({
          checkedSpecPrice: checkedProduct.curPrice,
          soldout: false
        });
        this.notify.hide();
      } else {
        this.setData({
          checkedSpecPrice: this.data.goods.currentPrice,
          soldout: true
        });
        this.soldoutNotify();
      }
      ...
      this.setData({
        checkedSpecText: '规格数量选择',
        checkedSpecPrice: this.data.goods.originalPrice,
        soldout: false
      });
      this.notify.hide();
    }
  ...
  getCheckedProductItem: function(key) {
    return this.data.stockList.filter(function(v) {
  ...
  onShow: function() {
    // 页面显示
    var that = this;
    if (wx.getStorageSync("token").userId != undefined) {
  ...
  addFast: function() {
    ...
    if (this.data.openAttr == false) {
      ...
      //立即购买
      util.request(api.CartAdd + "?userId=" + wx.getStorageSync("token").userId, {
          goodsSn: this.data.goods.goodsNum,
  ...
  cutNumber: function() {
    this.setData({
      count: (this.data.count - 1 > 1) ? this.data.count - 1 : 1
    });
  },
  addNumber: function() {
    this.setData({
